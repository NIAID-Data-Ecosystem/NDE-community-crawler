...
- `scrapers/*.py` — per-forum scrapers for no-auth / credentialed APIs (Reddit OAuth, Bioconductor Discourse)
...
- **Cloudflare-protected** (Biostars, SEQanswers) — go through `agent/fetch.py`, which clears the JS challenge with no API key. This supersedes the old "needs `BIOSTARS_API_KEY`" / "skip SEQanswers" approach.
